...
class ScheduleScreen extends StatelessWidget {
  ...
  /// Check if user can create exchange request for this schedule
  bool _canCreateExchangeRequest(ScheduleModel schedule) {
    // Debug logging
    print('DEBUG _canCreateExchangeRequest for ${schedule.title}:');
    print('  - startDateTime: ${schedule.startDateTime}');
    print('  - now: ${DateTime.now()}');
    print('  - isFuture: ${schedule.startDateTime.isAfter(DateTime.now())}');
    print('  - status: ${schedule.status}');
    print('  - isCompleted: ${schedule.isCompleted}');
    print('  - scheduleStatus: ${schedule.scheduleStatus}');
    print('  - workStatus: ${schedule.workStatus}');
    
    // Check if schedule is not completed (both check-in and check-out done)
    if (schedule.isCompleted == true) {
      print('  - REJECTED: Schedule is completed');
      return false;
    }

    // Check if schedule database status is active
    if (schedule.status != 'active') {
      print('  - REJECTED: Status is not active: ${schedule.status}');
      return false;
    }

    // Allow exchange for all active, non-completed schedules
    // The RPC function will validate if the schedule hasn't started yet
    print('  - ACCEPTED: Can create exchange request');
    return true;
  }

  /// Show exchange request dialog
      ...
      AlertDialog(
        ...
        title: Row(
          children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: AppConstant.accentColor.withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Icon(
                Icons.swap_horiz,
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          '${TimezoneUtils.formatTime12Hour(schedule.startDateTime)} - ${TimezoneUtils.formatTime12Hour(schedule.endDateTime)}',
                          style: TextStyle(
                            fontSize: 14,
                            color: AppConstant.textSecondary,
                          ),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(Icons.info_outline, size: 16, color: AppConstant.accentColor),
                        const SizedBox(width: 6),
                        Text(
                          'How it works:',
                          style: TextStyle(
                    ...
                    const SizedBox(height: 8),
                    Text(
                      '1. Select an available employee to exchange with\n2. Provide a reason for the exchange\n3. Submit your request to admins\n4. Wait for approval or rejection',
                      style: TextStyle(
                        fontSize: 13,
                        color: AppConstant.textSecondary,
                        height: 1.4,
        ...
        actions: [
          TextButton(
            onPressed: () => Get.back(),
            style: TextButton.styleFrom(
              foregroundColor: AppConstant.textSecondary,
            ),
          ...
          ElevatedButton(
            onPressed: () {
              Get.back();
              _navigateToExchangeRequestScreen(schedule);
            },
